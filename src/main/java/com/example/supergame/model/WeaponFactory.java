...
public class WeaponFactory {
    ...
    public Weapon createWeapon(List<WeaponName> weaponNames) {
//        int weaponTypeSelector = (int) (Math.random() * ((5 + 1)));
        int weaponTypeSelector = 1;
        if (weaponTypeSelector < 2) {
            return createRandomMeleeWeapon(weaponNames);
        } else if (weaponTypeSelector < 4) {
            return createRandomRangeSecondaryWeapon(weaponNames);
        } else {
            return createRandomRangePrimaryWeapon(weaponNames);
        }
    }

    //Creates a random melee weapon
    public MeleeWeapon createRandomMeleeWeapon(List<WeaponName> weaponNames) {
        MeleeWeapon meleeWeapon = new MeleeWeapon(null, 0, 0, 100, WeaponCategory.MELEE,
                raritySelection(), 0);

        setWeaponStats(meleeWeapon);
        setWeaponName(meleeWeapon, weaponNames);
        increaseDamageDependingOnWeaponType(meleeWeapon);
        return meleeWeapon;
    }

    //Creates a random Secondary Weapon
    public RangeWeapon createRandomRangeSecondaryWeapon(List<WeaponName> weaponNames) {
        ...
        setWeaponName(secondaryWeapon, weaponNames);
